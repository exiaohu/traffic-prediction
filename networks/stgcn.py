...
class SpatialConvLayer(nn.Module):
    ...
    def forward(self, inputs: Tensor, cheb_filters: Tensor) -> Tensor:
        ...
        return torch.relu(outputs + x_res)
...
class STGCN(nn.ModuleList):
    def __init__(self,
                 ...
                 dropout: float):
        super(STGCN, self).__init__([STConvBlock(k_hop, t_cnv_krnl_sz, n_node, dim, dropout) for dim in dims])
